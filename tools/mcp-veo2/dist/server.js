...
import { generateVideoFromText, generateVideoFromImage, generateImage, generateVideoFromGeneratedImage, listGeneratedVideos, listGeneratedImages, getImage } from './tools/generateVideo.js';
...
export function createServer() {
    ...
    server.resource('videos', videoResourceTemplate, {
    ...
    server.resource('image-templates', imagePromptsResource.uri, {
    ...
    server.tool('generateVideoFromText', 'Generate a video from a text prompt', {
    ...
    server.tool('generateVideoFromImage', 'Generate a video from an image', {
    ...
    server.tool('generateImage', 'Generate an image from a text prompt using Google Imagen', {
        prompt: z.string().min(1).max(1000),
        numberOfImages: z.number().min(1).max(4).default(1),
        includeFullData: z.union([z.boolean(), z.string()]).default(false),
    }, generateImage);
    // Register the image-to-video generation with generated image tool
    server.tool('generateVideoFromGeneratedImage', 'Generate a video from a generated image (one-step process)', {
        prompt: z.string().min(1).max(1000),
    ...
    server.tool('listGeneratedVideos', 'List all generated videos', listGeneratedVideos);
    // Register the get image tool
    server.tool('getImage', 'Get a specific image by ID', {
        id: z.string().min(1),
        includeFullData: z.union([z.boolean(), z.string()]).default(true),
    }, getImage);
    // Register the list images tool
    server.tool('listGeneratedImages', 'List all generated images', listGeneratedImages);
