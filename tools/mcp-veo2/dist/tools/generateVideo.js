 ...
 */
async function saveGeneratedImage(imageBytes, prompt, mimeType = 'image/png') {
    try {
...
export async function generateVideoFromImage(args) {
    try {
        ...
        // Generate the video
        const result = await veoClient.generateFromImage(imageData, args.prompt, config, options, mimeType);
        // Prepare response content
        const responseContent = [];
        // If includeFullData is true and we have video data, include it in the response
        if (args.includeFullData && result.videoData) {
...
export async function generateImage(args) {
    try {
        ...
        const { id, filepath } = await saveGeneratedImage(generatedImage.image.imageBytes, args.prompt, 'image/png');
        // Prepare response content
        const responseContent = [];
...
export async function generateVideoFromGeneratedImage(args) {
    try {
        ...
        // Save the generated image to disk
        const { id: imageId, filepath: imageFilepath } = await saveGeneratedImage(generatedImage.image.imageBytes, args.prompt, 'image/png');
        // Use the generated image to create a video
        const videoPrompt = args.videoPrompt || args.prompt;
        const result = await veoClient.generateFromImage(generatedImage.image.imageBytes, videoPrompt, videoConfig, options, 'image/png');
        // Prepare response content
        const responseContent = [];
 ...
 * @returns The tool result
 */
export async function listGeneratedVideos() {
    try {
        // Get all videos
